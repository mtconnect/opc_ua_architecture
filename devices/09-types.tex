% Generated 2019-10-31 19:14:08 -0700
...
The condition type is a derived from the UA \uamodel{ContitionType}. When the \mtmodel{Warning} or \mtmodel{Fault} state occurs, an \mtuamodel{MTConditionEventType} \uamodel{Event} is created and with the \mtmodel{ActiveState} set to \uamodel{True} and \uamodel{Retain} set to \uamodel{True}.
...
A new \uamodel{NodeId} will be created for every unique instance of the MTConnect \mtmodel{Condition} reported.  When the \mtmodel{Condition} goes back to Normal, the \mtmodel{ActiveState} is set  to \uamodel{False} and \uamodel{Retain} is also set to \uamodel{False} with the \uamodel{NodeId} of the associated \mtmodel{Condition}. If multiple MTConnect \mtmodel{Condition}s have been cleared at the same time, all currently active \mtuamodel{MTConditionEventType} \uamodel{Event}s will need to be created as inactive.

The \mtuamodel{MTConditionEventType} must set the \uamodel{BaseEvent} \uamodel{SourceId} to the related \mtuamodel{MTConditionType} that represents the meta-data for this Condition.

The \mtuamodel{MTConditionEventType} will never be instantiated in the \uaterm{AddressSpace} as an \uamodel{Object}. 
...
Has\-Component & Object & <MT\-Condition> & \multicolumn{2}{l|}{MTConditionType} & Mandatory \\
...
\begin{table}[ht]
\centering 
  \caption{\texttt{MTConditionEventType::ActiveState} Values}
\tabulinesep=3pt
