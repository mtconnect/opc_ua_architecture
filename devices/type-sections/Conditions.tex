...
In MTConnect, the \gls{MTCondition} provides the meta-data describing the \mtmodel{type}, \mtmodel{subType}, and other properties required by the MTConnect \gls{MTDataItem}. In response to a \gls{sample}, the \glspl{MTCondition} behaves like an event stream. In response to a \gls{current}, all active conditions are reported, even if there are multiple active conditions for a given type. Usually, an \gls{MTEvent} can only have one state at a time; the \gls{MTCondition} is different and can have multiple states and values, as in the case where there are multiple system alarms for a component.
