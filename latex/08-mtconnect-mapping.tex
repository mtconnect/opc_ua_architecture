...
\centering 
  ...
  \label{table:condition-states}
\fontsize{9pt}{11pt}\selectfont
\tabulinesep=3pt
\begin{tabu} to 6in {|X|X|X|X|X[3]|} \everyrow{\hline}
\hline
\rowfont\bfseries {Branch Id} & {Active} & {Retain} & {Native Code} & {Message} \\
\tabucline[1.5pt]{}
NULL & false & false & NULL & NULL \\
0 & true & true & PLC--154 & PIN SENSOR MALF \\
1 & true & true & PLC--155 & WORK NO. ERROR(0 OR >9999) \\
2 & true & true & PLC--157 & WARMING UP!!! \\
0 & false & false & PLC--154 & PIN SENSOR MALF \\
2 & false & false & PLC--157 & WARMING UP!!! \\
1 & false & false & PLC--155 & WORK NO. ERROR(0 OR >9999) \\
NULL & false & false & NULL & NULL \\
\end{tabu}
\end{table}

MTConnect uses the \mtmodel{nativeCode} to determine the uniqueness of each activation of the Condition. If the \mtterm{Path} component has a \gls{MTDataItem} with a \gls{type} of \mtmodel{LOGIC_PROGRAM} and there  listing~\ref{lst:path-logic-program-condition-1}. The first fault occurred on \mtmodel{PLC-154} native code and creates the first branch. If there is only one branch, then the branch ID will not be required and may remain \uamodel{NULL}.

\begin{lstlisting}[firstnumber=last,escapechar=|,%
    caption={Path Logic Program Error},label={lst:path-logic-program-condition-1}]
  <ComponentStream componentId="a4a7bdf0" component="Path" name="P1">
    <Condition>
...
The second \mtmodel{Fault} is given in Listing~\ref{lst:path-logic-program-condition-2} where a second PLC alarm is active. The native code is different than the previous condition, so a second branch must be created. The branch will assigned the number 1 for this instance.

\begin{lstlisting}[firstnumber=last,escapechar=|,%
    caption={Path Logic Program Error},label={lst:path-logic-program-condition-2}]
  <ComponentStream componentId="a4a7bdf0" component="Path" name="P1">
    <Condition>
...
The warning in Listing~\ref{lst:path-logic-program-condition-3} indicates the machine is warming up and other operations are disabled. This condition has another \mtmodel{nativeCode} and therefor, like the previous condition, must create another branch. In this instance the \uamodel{BranchId} will be 2. 

\begin{lstlisting}[firstnumber=last,escapechar=|,%
    caption={Path Logic Program Error},label={lst:path-logic-program-condition-3}]
  <ComponentStream componentId="a4a7bdf0" component="Path" name="P1">
    <Condition>
...
\begin{lstlisting}[firstnumber=last,escapechar=|,%
    caption={Path Logic Program Error},label={lst:path-logic-program-condition-5}]
  <ComponentStream componentId="a4a7bdf0" component="Path" name="P1">
    <Condition>
      <Normal sequence="5467" timestamp="2018-10-31T20:51:19.9981Z" dataItemId="a557d330" type="LOGIC_PROGRAM" nativeCode="PLC-157"/>
    </Condition>
  </ComponentStream>
\end{lstlisting}

Listing~\ref{lst:path-logic-program-condition-6} is the final \mtmodel{Normal} transition that clears all the currently active conditions and indicates that the \mtmodel{Conditions} is now inactive or cleared and back to a \mtmodel{Normal} state.

\begin{lstlisting}[firstnumber=last,escapechar=|,%
    caption={Path Logic Program Error},label={lst:path-logic-program-condition-6}]
  <ComponentStream componentId="a4a7bdf0" component="Path" name="P1">
    <Condition>
      <Normal sequence="5467" timestamp="2018-10-31T20:51:19.9981Z" dataItemId="a557d330" type="LOGIC_PROGRAM"/>
    </Condition>
  </ComponentStream>
\end{lstlisting}
...
The \mtuatype{MTMessageType} is a data item with an extended type that uses the \mtuatype{MessageEventDataType} to provide the \mtterm{NativeCode} and the \mtterm{Text} of the message. The \gls{CDATA} will be mapped to the \mtterm{Text} field and the \mtterm{NativeCode} will be mapped if present.
