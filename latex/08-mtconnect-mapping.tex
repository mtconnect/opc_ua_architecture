...
{\color{red} The alternative architecture is to have the message as an \gls{Event} subtype, but the dual instantiation of the \mtuatype{MTMessageType} as an \gls{Object} and an \gls{Event} proved to be very complicated to implement. This can be reconsidered. One option to consider is firing an event, similar to state transitions, every time the value changes. Will look into the process of creating an event for this purpose. May also be able to use an existing event. In that case the MTMessage instance will become an event source.}
