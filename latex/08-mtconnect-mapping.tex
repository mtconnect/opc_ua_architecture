...
The MTConnect \gls{MTDataItem} represents multiple OPC UA \uamodel{DataVariableType}s as well as UA \uamodel{ConditionType}s as described in parts \cite{UAPart8} and \cite{UAPart9} respectively. The mapping rules MUST be applied in strict sequence as follows:
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \begin{enumerate}
      ...
      \item If the \gls{category} is \mtmodel{EVENT}; apply the rules strictly in sequence as follows:
      \begin{enumerate}
        \setlength\itemsep{1em}
        ...
        \item If the \var{classType} matches one of the  \mtuamodel{ControlledVocabDataItemTypes}, the MTConnect UA \gls{TypeDefinition} is \mtuatype{MTControlledVocabEventType}. The \uamodel{EnumStrings} will be assigned as follows:
        \newline
        
        Each of the \glspl{ObjectType} matching the \var{classType} for \mtuamodel{ControlledVocabDataItemTypes} will have a \gls{Property} \uamodel{EnumStrings}. The \uamodel{EnumStrings} will reference the \uamodel{Enumeration} with the values of the \gls{ControlledVocab}.
  ...
  \item Assign the \mtuatype{HasMTClassType} and \mtuatype{HasMTSubClassType} \glspl{Reference} using the rules in sequence as follows:
  \begin{enumerate}
...
The \mtmodel{Controller} has two \mtmodel{DataItems}, \mtmodel{Message} and \mtmodel{EmergencyStop} as shown in Figure~\ref{fig:controller-component}. The \mtuatype{MTMessageType} is an \uamodel{Variable} where the data type is a \mtuatype{MessageEventDataType}.
...
Table~\ref{table:reset-trigger-to-status-code} provides the OPC UA \uamodel{StatusCode}s suggested values that need to be added in the working group to represent the MTConnect \mtmodel{ResetTrigger} model. The OPC Foundation TAC is asked to consider the addition of these additional status codes to support this capability in the MTConnect standard.

\begin{table}[ht]
  \centering
  \caption{MTConnect \mtmodel{ResetTrigger} to OPC UA \uamodel{StatusCode} mapping}
  \label{table:reset-trigger-to-status-code}
  \tabulinesep=3pt
  \fontsize{9pt}{11pt}\selectfont
  \begin{tabu} to 6in {|X|X|X[2]|} \everyrow{\hline}
    \hline
    \rowfont\bfseries {\mtmodel{ResetTrigger}} & {\uamodel{StatusCode}} & {Description} \\
    \tabucline[1.5pt]{}
    \mtmodel{ACTION_COMPLETE} & \uamodel{Good_ActionCompleteReset} & The value of the Data Entity that is measuring an action or operation was reset upon completion of that action or operation. \\
    \mtmodel{ANNUAL} & \uamodel{Good_AnnualReset} & The value of the Data Entity was reset at the end of a 12-month period. \\
    \mtmodel{DAY} & \uamodel{Good_DayReset} & The value of the Data Entity was reset at the end of a 24-hour period. \\
    \mtmodel{MAINTENANCE} & \uamodel{Good_MaintenanceReset} & The value of the Data Entity was reset upon completion of a maintenance event. \\
    \mtmodel{MANUAL} &  \uamodel{Good_ManualReset} & The value of the Data Entity was reset based on a physical reset action. \\
    \mtmodel{MONTH} & \uamodel{Good_MonthReset} & The value of the Data Entity was reset at the end of a monthly period. \\
    \mtmodel{POWER_ON} & \uamodel{Good_PowerOnReset} & The value of the Data Entity was reset when power was applied to the piece of equipment after a planned or unplanned interruption of power has occurred. \\
    \mtmodel{SHIFT} & \uamodel{Good_ShiftReset} & The value of the Data Entity was reset at the end of a work shift. \\
    \mtmodel{WEEK} & \uamodel{Good_WeekReset} & The value of the Data Entity was reset at the end of a 7-day period. \\
  \end{tabu}
\end{table}
...
The following section provides the rules for taking data from the \mtterm{MTConnectStream} XML Document and represent as a series of updates to the \gls{Variable} and generate \glspl{Event} for the \mtuatype{MTConditionType} and \mtuatype{MTMessageType} \uaterm{DataItem}.
...
MTConnect allows for multiple \glspl{MTCondition}s to be active at the same time. This is similar to the concept of \uaterm{Branching} in OPC UA. In MTConnect the the attribute \mtmodel{nativeCode} is used to indicate the association to the same \uaterm{Branch} and is mapped to a branch id as illustrated in Figure~\ref{fig:condition-branching}. In this diagram there are three branches--0, 1, and 2--that are represent PLC alarms and are mapped to the \mtmodel{LOGIC_PROGRAM}. Each of these unique \mtmodel{nativeCode}s is mapped to a unique branch ID and is tacked separately. Only when the final condition transitions to normal, indicated by a \mtterm{Normal} with no \mtterm{nativeCode}, is the \gls{MTCondition} considered cleared and completely inactive.
...
Table~\ref{table:ua-condition-states} represents the state transitions of the key OPC UA \uamodel{Condition} model and the \mtuatype{MTConditionType}. The text that follows will refer to this table and the MTConnect \gls{xml} to illustrate the expected behavior.

\begin{table}[ht]
\centering 
  \caption{\mtuamodel{LogicProgramCondition} States}
...
\begin{tabu} to 6in {|X[0.25,r]|X[r]|X|X|X|X[3]|} \everyrow{\hline}
\hline
\rowfont\bfseries {Seq} & {Branch Id} & {Active} & {Retain} & {Native Code} & {Message} \\
\tabucline[1.5pt]{}
\rownumber & NULL & false & false & NULL & NULL \\
\rownumber & 0 & true & true & PLC--154 & PIN SENSOR MALF \\
\rownumber & 1 & true & true & PLC--155 & WORK NO. ERROR(0 OR >9999) \\
\rownumber & 2 & true & true & PLC--157 & WARMING UP!!! \\
\rownumber & 0 & false & false & PLC--154 & PIN SENSOR MALF \\
\rownumber & 2 & false & false & PLC--157 & WARMING UP!!! \\
\rownumber & 1 & false & false & PLC--155 & WORK NO. ERROR(0 OR >9999) \\
\rownumber & NULL & false & false & NULL & NULL \\
\end{tabu}
\end{table}

MTConnect uses the \mtmodel{nativeCode} to determine the uniqueness of each activation of the Condition. If the \mtterm{Path} component has a \gls{MTDataItem} with a \gls{type} of \mtmodel{LOGIC_PROGRAM} and there listing~\ref{lst:controller-component}.
...
The first fault occurred with the \mtmodel{nativeCode} \mtmodel{PLC-154} and creates the first branch as shown in Listing~\ref{lst:path-logic-program-condition-1}. If there is only one branch, then the \uamodel{BranchId} will not be required and may remain \uamodel{NULL}. In this example we are setting the \uamodel{BranchId} to 1 as shown in Row~2 of Table~\ref{table:ua-condition-states}. A \mtmodel{Fault} indicates a situation where the piece of equipment is no longer able to continue functioning and needs manual intervention.
...
The warning in Listing~\ref{lst:path-logic-program-condition-3} indicates the machine is warming up and other operations are disabled. This condition has another \mtmodel{nativeCode} and therefor, like the previous condition, must create another branch. In this instance the \uamodel{BranchId} will be 2. The \mtmodel{Warning} will be represented in UA as a \uamodel{severity} and represents something that is of concern but not stopping the process. The warning is given by Row~4 of Table~\ref{table:ua-condition-states}.
...
In Listing~\ref{lst:path-logic-program-condition-5}, when the sensor malfunction is reset, the first condition will be returned to an inactive state. It is indicated by \mtmodel{Normal} and a native code of \mtmodel{PLC-154}. Since the other two conditions are still active, the \mtuatype{MTConditionType} is not inactive. In MTConnect, a \mtmodel{current} request would indicate that there is a Fault and a Warning currently active for this \mtmodel{Condition}. Similar to the previous state, Table~\ref{table:ua-condition-states} clears the active state of this branch on Row~6.
...
The \mtuatype{MTMessageType} is a data item with an extended type that uses the \mtuatype{MessageEventDataType} to provide the \mtterm{NativeCode} and the \mtterm{Text} of the message. The \gls{CDATA} will be mapped to the \mtterm{Text} field and the \mtterm{NativeCode} will be mapped to the field of the UA \uamodel{DataType} if it is provided as an \mtmodel{attribute}.
