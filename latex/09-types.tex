% Generated 2019-03-02 18:09:29 -0800
...
Organizes & Object & Assets & MT\-Asset\-Type[] & Folder\-Type & Optional \\
\end{tabu}
\end{table} 
...
The message is a sub-type of the \uaterm{DataVariableType} using the \mtuatype{MessageEventDataType} to 
...
There are two Information Models used to represent a Cutting Tool, an \mtuatype{MTCuttingToolArchetypeType} and a \mtuatype{MTCuttingToolType}. 
The \mtuatype{MTCuttingToolArchetypeType} represent the static Cutting Tool geometries and nominal values as one would 
expect from a tool catalog and the \mtuatype{MTCuttingToolType} represents the use or application of the tool on the shop 
floor with actual measured values and process data. In this way, a generic definition of the Cutting Tool can coexist with a specific 
assembly information model with minimal redundancy of data.
...
Organizes & Object & Cutting\-Items & MT\-Cutting\-Item\-Type[] & MT\-Cutting\-Items\-Folder\-Type & Optional \\
\end{tabu}
\end{table} 
...
\begin{itemize}
\item \texttt{ProcessSpindleSpeed::MTCuttingToolConstraintType:} The \mtmodel{ProcessSpindleSpeed} \must be specified in revolutions/minute (RPM). The \gls{CDATA}
\may contain the nominal process target spindle speed if available. The \mtmodel{maximum} and
\mtmodel{minimum} speeds \may be provided as attributes. If ProcessSpindleSpeed is provided, at
least one value of \mtmodel{maximum}, \mtmodel{nominal}, or \mtmodel{minimum} \must be specified.

\item \texttt{ProcessFeedRate::MTCuttingToolConstraintType:} The \mtmodel{ProcessFeedRate} \must be specified in millimeter/second. The \gls{CDATA}
\may contain the nominal process target spindle speed if available. The \mtmodel{maximum} and
\mtmodel{minimum} speeds \may be provided as attributes. If ProcessSpindleSpeed is provided, at
least one value of \mtmodel{maximum}, \mtmodel{nominal}, or \mtmodel{minimum} \must be specified.
