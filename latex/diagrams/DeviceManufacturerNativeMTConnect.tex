...
\centering\scalebox{0.8}{\begin{tikzpicture}[scale = 3, node distance = 2cm, auto]
    ...
    \path [line] (mtcopcgateway) |- (genericopcuaclient);
    \path [line] (mtcopcgateway) |- (mtcopcclient);
    \path [line] (genericopcuaclient) -| (mtcopcgateway);
    \path [line] (mtcopcclient) -| (mtcopcgateway);
    \path [line] (parentnode) -- (mtcopcgateway);
    \path [line] (mtcopcgateway) -- (parentnode);
    \path [line] (mtcapplication) -- (mtcagent);
    \path [line] (mtcagent) -- (mtcapplication);
    \end{tikzpicture}
    }
\caption{Device Manufacturer with Native MTConnect Agent}
 \label{fig:devicemanufacturernativemtconnect}
\end{figure}