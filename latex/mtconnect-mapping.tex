    ...
    \item Evaluate \xmlelement{DataItem} attributes as follows:
        ...
        \item If the category is \texttt{EVENT}; apply the following rules:
            ...
            \item If the \uaterm{ClassType} matches one of the  \mtuamodel{ControlledVocabDataItemTypes}, the MTConnect UA type is \mtuatype{MTControlledVocabEventType}. The \uaterm{EnumValues} will be assigned as follows:
                \begin{enumerate}
                \item Each of the \uaterm{ClassType}s will have a property \mtterm{EnumValues}. The \uaterm{EnumValues} must reference the \mtterm{EnumValues} property of the \uaterm{ClassType}.
                \end{enumerate}
...
The OPC UA \uaterm{DataVariableType}s are monitored items and have three attribtues that are updated when the value changes. They are the \uaterm{Value} in the instance of the \uaterm{DataVariable}, the \uaterm{SourceTimestamp}, and the \uaterm{Quality}. When the value is given, not \texttt{UNAVAILABLE}, the \uaterm{Quality} will be set to the Status Code of \uaterm{Good}, the \uaterm{SourceTimestamp} will be set to the \xmlattribute{timestamp} attribute and the \uaterm{Value} set to the contents of the \xmlelement{CDATA} converted to a \mtterm{Double} precision value.
...
The instance of the \mtuatype{MTConditionType} will contain the latest state of the \mtterm{Condition} as they are activated. Each time an MTConnect \mtterm{Condition} activates or deactivates, an \uaterm{Condition} \uaterm{Event} will be created associated with the instance if the \mtuatype{MTConditionType} \uaterm{NodeId}. 
