...
The \mtterm{Controller} has two \mtterm{DataItems}, a \mtterm{Message} and \mtterm{EmergencyStop} as shown in Figure~\ref{fig:controller-component}. The \mtuatype{MTMessageType} is of special note since it is not a \uaterm{VariableType}, but it is an \uaterm{EventType} and decended from the \uaterm{SystemEventType} class described in \cite{UAPart5}. {\color{red} Does the message require a special relation like the Condition and the HasCondition reference?}

In MTConnect, the \mtterm{Controller} has common data and information relating to all \mtterm{Path}s of operation. The \mtterm{Path} element below has the specific information relating to the execution of the part program and the path related motion and conditions. In some simple machines, the \mtterm{Path} will be merged to the \mtterm{Controller}. These relationships are covered in \cite{MTCPart2}.
...
Figure~\ref{fig:path-component} illustrates the \mtterm{Path} component with \mtterm{DataItems}. Many of the \mtterm{DataItems} types have been covered in prior examples, so the \mtterm{Path} will only focus on the \mtterm{DataItems} that have differentiated features. 
