...
The \mtterm{UMLAssociations} are used to represent OPC UA References. \uaterm{UA Properties} represented using the UA \opcuaterm{HasProperty} relationships are given as UML Attributes (not to be confused with the \uaterm{OPC UA Attribute}). UML allows for additional information as follows:  the multiplicity of a property, the data type of the property, are specified.
...
A reference that has a stereotype of \textit{<<Organizes>>}, shown by the \texttt{Components} recursive relationship in the \mtconnectterm{MTComponentType}, implies an intermediate object of type \opcuaterm{FolderType} with the \gls{BrowseName} of the relationship point on the source side. The far side is not constrained within the \opcuaterm{FolderType} but given in the documentation as the expected contents of the folder.
...
Common elements, like \xmlelement{Components} or \xmlelement{Compositions}, that provide structure that are mapped to \gls{FolderType} objects have common browse names. These will be prefixed with the context of the parent component. For example, \texttt{Controller.Components} will refer to an \uaterm{Object} of type \gls{FolderType} with the \gls{BrowseName} \texttt{Components}. The prefix before the period (\texttt{.}) will be ignored for the \gls{BrowseName}. {\color{red} This rule will be removed when UML Diagrams are converted to TIKz as this is a requirement of UML modeling to have unique object names--UML modeling tools require unique object names.}
...
The first concern of the MTConnect OPC UA companion specification is the \gls{MTDevice} model covered in MTConnect \cite{MTCPart2} and \cite{MTCPart3}. The top-level \gls{MTComponent} of any MTConnect information model is the \gls{MTDevice}. A \gls{MTComponent} represents a logical part or a collection of parts of a piece of equipment. The \glspl{MTDataItem} represent information that is communicated from \glspl{MTComponent}, and the representation and communication of the information are covered in \cite{MTCPart3}. The \mtconnectterm{Compositions} are the lowest level of contextualization for MTConnect \glspl{MTComponent} that do not have any structure but can be associated with \glspl{MTDataItem} to provide additional context. 

The \\gls{Configuration} is a collection of information about the component that provides more detail about its capabilities. The standard has only specified the \mtconnectterm{SensorConfiguration} at this point.
...
The type names in UA will require the upper case with underscore name to be converted to \gls{PascalCase}. XML Attributes are converted from \gls{LowerCamelCase} to \gls{PascalCase} as well when mapping to UA Property \opcuaterm{BrowseName}s.
...
The rules for determining the \gls{BrowseName} for the MTConnect \mtterm{Component} are as follows:

\begin{enumerate}
  \item If the \mtterm{Component} is \xmlelement{Device}, the \gls{BrowseName} is the \xmlattribute{name} attribute of the \xmlelement{Device} and the UA \uaterm{TypeDefinition} is \texttt{MTDeviceType}.
  \item The browse name is the \xmlelement{QName} of the XML Element of the \mtterm{Component}. \label{item:component-browse-name}
  \item The \uaterm{HasTypeDefinition} references a type that is created by appending the word \texttt{Type} to the \xmlelement{QName} of the \mtterm{Component} element. For example, a \xmlelement{Controller} will have a \gls{BrowseName} of \texttt{Controller} and a \uaterm{HasTypeDefinition} referencing \texttt{ControllerType}.
  ...
  \item For example, if there are two \xmlelement{Path} \mtterm{Components} with names \xmlattribute{name="P1"} and \xmlattribute{name="P2"}, the \glspl{BrowseName} will be as follows: \texttt{Path[P1]} and \texttt{Path[P2]} where each \uaterm{HaveTypeDefinition} of \texttt{PathType}.
\end{enumerate}

The rules for \mtterm{Composition} elements are as follows:

\begin{enumerate}
  \item  \mtterm{Composition} elements in the MTConnect model will be represented instantiated with the \gls{BrowseName} of the \mtterm{Composition} \xmlattribute{type} converted to \textit{PascalCase}. \label{item:composition-browse-name}
  ...
  \item For example, a component has two \xmlattribute{type="TANK"}s with the names \xmlattribute{name="top"} and \xmlattribute{name="bottom"}, the \glspl{BrowseName} will be \texttt{Tank[top]} and \texttt{Tank[bottom]} respectively.
...
The MTConnect \gls{MTDataItem} represents multiple OPC UA \uaterm{DataVariableType}s as well as UA \uaterm{ConditionType}s as described in parts \cite{UAPart8} and \cite{UAPart9} respectively. The mapping rules are as follows:

\begin{enumerate}
  \item Determine the \gls{MTDataItem} \uaterm{Type} for the \uaterm{HasTypeDefinition} \uaterm{Reference}:
  \begin{itemize}
  ...
  \item Format the \gls{BrowseName} for the \mtterm{Sample} and \mtterm{Event} \mtterm{DataItem}s
  \begin{enumerate}[after=\vspace{\baselineskip}]
    ...
    \item If the \xmlattribute{representation} attribute is given and it has a value other than the default \texttt{VALUE}, the \gls{BrowseName} will be have the \textit{PascalCase} of the \xmlattribute{representation} appended to the \gls{BrowseName}  as specified in \ref{item:browse-name} . For example, a \dataitem{AMPERAGE} with \xmlattribute{representation="TIME_SERIES"} will have a \gls{BrowseName} of \texttt{AmperageTimeSeries}. \label{item:representation}.
    \item If the \xmlattribute{statistic} is given, the \textit{PascalCase} of the \xmlattribute{statistic} attribute will be prepended to the \gls{BrowseName}  as specified in \ref{item:representation}. For example, if the \xmlattribute{statistic="AVERAGE"} for a \gls{MTDataItem} of \dataitem{AMPERAGE} and a \xmlattribute{representation} of \dataitem{TIME_SERIES}, the \gls{BrowseName} will be  \texttt{AverageAmperageTimeSeries}. \label{item:statistic}
    \item If two browse names are identical for the same component, the \xmlattribute{name} must be used to differentiate between the data items by placing the \xmlattribute{name} in square brackets (\texttt{[]}). For example, if there are two \dataitem{TEMPERATURE} data items with the attributes \xmlattribute{name="front"} and \xmlattribute{name="back"} the \glspl{MTDataItem} will have the \glspl{BrowseName} \texttt{Temperature[front]} and \texttt{Temperature[back]} respectively. \label{item:data-item-name}
  \end{enumerate} 

  \item Format the \gls{BrowseName} for the \mtterm{Condition} \mtterm{DataItem}s
  \begin{enumerate}
...
Every device must have a \opcuaterm{HasNotifier} relationship with the \uaterm{Server} UA Object as the source to enable conditions and event notification. The \opcuaterm{HasNotifier} relationships are flowed down through the component hierarchy where the \opcuaterm{Component}s each have a \uaterm{HasNotifier} relationship with the parent. The MTConnect \glspl{MTDataItem} will have require a \uaterm{HasEventSource} relationship with the parent \mtterm{Component} when the \gls{MTDataItem} is associated with a \mtterm{Condition} where the \texttt{Source} element of the \mtterm{Condition} references the \gls{MTDataItem} id. 
...
Every MTConnect \mtconnectterm{Data\-Item} has two references that provide the semantic meaning in the UA address space. This model is very similar to the \texttt{Condition\-Class\-Id} relationship for the OPC UA \opcuaterm{Condition\-Type}. These \texttt{Class\-Type}s are inherited from the same parent so they can be used for both the \mtterm{HasMTClassType} for all \glspl{MTDataItem} as well as the \uaterm{ConditionClassId} in \uaterm{Conditions} since MTConnect \mtterm{Conditions} are also descended from the \uaterm{ConditionType} hierarchy.
...
The \glspl{MTDataItem} for \texttt{ActualPosition} and \texttt{Load} are both of \mtterm{category} \mtterm{SAMPLE}; they contain numeric values are always mapped to the \mtuatype{MTSampleType} which is derived from the UA \uaterm{AnalogItemType} defined in \cite{UAPart8}. The \gls{BrowseName} for the \glspl{MTDataItem} are composed of the related \mtterm{Composition} \mtterm{type}, \mtterm{subType}, and \mtterm{type} of the \gls{MTDataItem} represented in \textit{PascalCase}. See Section~\ref{sec:data-item-conventions} for details.
...
Figure~\ref{fig:rotary-c-amperage} illustrates the \mtterm{Amperage} \gls{MTDataItem} corresponding to the Listing~\ref{lst:rotary-c-example} Lines~\ref{line:rotary-c-amperage}-\ref{line:rotary-c-amperage-condition}. Unlike the previous example of a MTConnect \mtterm{Condition} relating to the MTConnect \mtterm{Component}, the \xmlelement{Source} element under the \xmlelement{DataItem} references the \gls{MTDataItem} with \xmlattribute{type} \dataitem{AMPERAGE}. When the \mtterm{source} refers to a \gls{MTDataItem} that is part of the same MTConnect \mtterm{Component}, the UA \uaterm{Variable} is given a \uaterm{Reference} \uaterm{HasEventSource} from the \mtterm{RotaryType} (Rotary[C]) and a \uaterm{HasCondition} \uaterm{Reference} is created beteen the \uaterm{Variable} to the \mtterm{MTConditionType}.

As stated above, the \uaterm{HasNotifier} \uaterm{References} must be established between all \mtterm{Components} back to the \mtuatype{MTDeviceType} that has a reference from the \uaterm{Server}. The \texttt{MotorAmperage} and \texttt{MotorAmperageCondition} demonstrate the naming rule where the Composition type is converted to \textit{PascalCase} and then prepended to the \gls{MTDataItem} \xmlattribute{type}. 
...
The Temperature \uaterm{Variable} also has a \xmlelement{Source} element that references the \mtterm{Sensor} with a \xmlelement{Configuration} of type \mtterm{SensorConfigurationType}. The \mtterm{SensorConfiguration} has information about the \mtterm{FirmwareVersion} and \mtterm{CalibrationDate} of the \mtterm{Sensor}. The \mtterm{Channels} represent the multiple inputs to the \mtterm{Sensor} unit. The first \mtterm{Channel} has a \mtterm{Description} and a \mtterm{CalibrationDate} of the probe attached to the \mtterm{Channel}. For more information on the \mtterm{Channel}, see MTConnect Devices \cite{MTCPart2}.
...
In this case the \mtterm{Composition} name is converted to \textit{PascalCase} and prepended to the \textit{PascalCase} of the \gls{MTDataItem} to create the base \uaterm{BrowseName}. Since there are two identical \uaterm{BrowseNames}, the \xmlattribute{name} is appended in square brackets (\texttt{[]}). The same is done for the \mtterm{Composition} \gls{BrowseName} and the \mtterm{Component} \gls{BrowseName} "\mtterm{Coolant}" giving \texttt{Coolant[high]} and \texttt{Coolant[low]}.
...
MTConnect flattens the structure when reporting data and organizes it by \mtterm{Device}, \mtterm{Component} and \mtterm{Category}; this is shown in  Listing~\ref{lst:linear-component-stream} Line~\ref{line:component-stream-1} with the \xmlelement{DeviceStream}, \xmlelement{ComponentStream}, and \xmlelement{Samples} elements (not shown are the \xmlelement{Events} and \xmlelement{Condition} elements that will be address later). When the data stream is initialized or when it disconnects, the MTConnect \mtterm{DataItem Value} is set to \texttt{UNAVAILABLE} as shown on Line~\ref{line:pos-unavilable}.
...
In Listing~\ref{lst:path-component-stream} on Line~\ref{line:prog-430}, the string value will be assigned to the \mtterm{Program} \gls{MTDataItem} in the \mtterm{Path} component. The following Line~\ref{line:pc-603} representing the \xmlelement{PartCount} will be converted to a numeric value, in this case an \uaterm{Int32} and assigned to the \uaterm{Value}.
...
MTConnect allows for multiple \mtterm{Condition}s to be active at the same time. This is similar to the concept of \uaterm{Branching} in OPC UA. In MTConnect the unique identifier that is used for the \uaterm{BranchId} is the attribute \mtterm{nativeCode}.

MTConnect uses the \xmlattribute{nativeCode} to determine the uniqueness of each activation of the Condition. If the \mtterm{Path} component has a \gls{MTDataItem} with a \texttt{type} of \texttt{MOTION_PROGRAM} and there are four syntax errors for lines 10, 120, 320, and 500; there are five faults reported by MTConnect for the Condition. The response to a \mtterm{current} request is shown in listing~\ref{lst:path-motion-program-condition}.
...
When the syntax error is fixed on line 120, there will be a \mtterm{Condition} streamed with a state of \mtterm{Normal} and a \mtterm{nativeCode} of \texttt{SYNTX-120}. The results of a \mtterm{current} request are shown in listing~\ref{lst:path-motion-program-condition-2}.
...
\mtterm{DataItems} with \mtterm{TIME_SERIES} \xmlattribute{representation} provides a mechanism to publish high frequency data by combining multiple observations into a single vector of values. The attribute \xmlattribute{sampleRate} is specified in the \mtterm{Devices} information model if it is immutable or for each entity with the attribute \xmlattribute{sampleRate} if it is variable. 
...
In MTConnect, the \gls{MTDataItem} element QName in the \mtterm{Streams} document appends the \textit{PascalCase} of the \xmlattribute{representation} to the \textit{PascalCase} of the \mtterm{type}. For example, \xmlattribute{type} \xmlattribute{VOLT_AMPERE} with the \xmlattribute{representation} of \xmlattribute{TIME_SERIES} becomes \xmlelement{VoltAmpereTimeSeries}. 
