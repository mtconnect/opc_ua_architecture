...
The device requires three data items as of MTConnect version 1.2. The \dataitem{AVAILABILITY} data item indicates if data is available from the device and the \dataitem{ASSET_CHANGED} and \dataitem{ASSET_REMOVED} data items represent the last asset inserted or updated and removed respectively. The Component adds Data items with a \uaterm{Has\-Component} relationship, and the BrowseName is constructed using the \mtmodel{type}, \mtmodel{sub\-Type}, and related \texttt{Composition} from \mtmodel{compositionId} if given. A complete example is given later in this section.
...
The \mtmodel{Components} element in Listing \ref{lst:linear-x-component} is represented as a UA \uaterm{Folder\-Type}. Within the folder, the MTConnect components are semantically identified by the element's \mtmodel{QName} with a type with the suffix \texttt{Type} appended, in this case the \gls{BrowseName}  \mtmodel{Axes} has a \opcuaterm{HasTypeDefinition} relaton of \texttt{AxesType}. 

\input{diagrams/mtconnect-mapping/linear-x-component.tex}

Figure~\ref{fig:linear-x-component} represents the UA Object model based on  Listing~\ref{lst:linear-x-component}. The \mtterm{Linear} X \mtterm{Axis}, mapped from \mtmodel{Linear} element in Listing~\ref{lst:linear-x-component}, has a \gls{BrowseName} composed of the \mtmodel{QName} of the element, and the \mtmodel{name} attribute appended and enclosed in square brackets \mtmodel{[X]}, giving the browse name of \texttt{Linear[X]} and a component type of \texttt{LinearType} using a \uaterm{HasTypeDefinition} \uaterm{Reference}.
