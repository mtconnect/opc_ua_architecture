...
When translating from MTConnect to OPC UA, the MTConnect abstractions of \dataitem{Data\-Item}s are converted using the OPC UA  \opcuaterm{Data\-Variable} abstractions as given in \cite{UAPart8}.  The relationships are mapped to multiple \opcuaterm{DataVariable} types where the category and the type determine the correct mapping. Conditions mapped to the OPC UA \opcuaterm{ConditionType} and require additional consideration regarding the notification hierarchy. 
...
For example, the object with \mtconnectterm{MTDeviceType} has \texttt{BrowseName}.
...
The data item class types and enumeration data types do not have \texttt{MT} prepended. Sub-types of the \texttt{MTComponentType} do not have \texttt{MT} prefixed with the exception of the \texttt{MTDeviceType}. All compositions that are sub-types of the \texttt{MTCompositionType} do not have any prefix.
...
The \texttt{Description} element provides some characteristics of the device, namely the manufacturer, serial-number and model. These are all optional as is the descriptive text that is contained in the \texttt{Description} element. The text contained in the \cdata{CDATA} of the \texttt{Description} element is mapped to the \texttt{Data} property of the \mtconnectterm{MTDescription}

The \texttt{Device} is mapped to an \mtconnectterm{MTDevice} object in the MTConnect namespace as shown in Figure \ref{fig:mazak-01-device-model}.
...
Conditions have special mapping rules that differ from the other data items. The conditions are sub-types of the OPC UA base type of \texttt{OffNormalAlarmType} with the \texttt{NormalState} node id of the NormalState mapped to the Normal instance of the UA \texttt{UAStateType}.
