...
The \texttt{AverageAmperage} on line~\ref{line:electric-average-amperage} has a \mtterm{ResetTrigger} that is added to the meta data indicating that the \mtterm{Average} statistic is taken until the current \mtterm{Action} has \mtterm{Completed} as defined by the process. This \mtterm{DataItem} will only update on when the \mtterm{ResetTrigger} fires. {\color{red} Need to add the status codes to represent the reset triggers as Good_...}.
