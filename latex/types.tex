% Generated 2018-11-03 01:08:16 -0700
...
\FloatBarrier
\subsubsection{Defintion of \texttt{ ConstraintDataType}}
  \label{type:ConstraintDataType}

\FloatBarrier

Provides the constraints on the range of values or the allowable values that can be represented in a data item. 
If a single Value is given in the Values field, the data item is considered to have a static value that
never changes.

\begin{table}[ht]
\centering 
  \caption{\texttt{ConstraintDataType} DataType}
  \label{data-type:ConstraintDataType}
\tabulinesep=3pt
\begin{tabu} to 6in {|l|l|} \everyrow{\hline}
\hline
\rowfont\bfseries {Field} & {Type}  \\
\tabucline[1.5pt]{}
\texttt{Values} & \texttt{String[]} \\
\texttt{Minimum} & \texttt{Double} \\
\texttt{Nominal} & \texttt{Double} \\
\texttt{Maximum} & \texttt{Double} \\
\end{tabu}
\end{table} 
...
\begin{itemize}
\item \texttt{Values::String:} Array of allowable values.

\item \texttt{Minimum::Double:} Set to NaN when not used.

\item \texttt{Nominal::Double:} Set to NaN when not used.

\item \texttt{Maximum::Double:} Set to NaN when not used.
...
HasProperty & Variable & Constraints & ConstraintDataType & PropertyType & Optional \\
