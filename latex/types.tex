% Generated 2018-11-18 20:12:11 -0800
...
MTConnect \mtuatype{MTComponentType}. The component types will be created once for all Component objects 
...
folder with a browse name of the Component QName and the \mtmodel{name} element if specified surrounded 
by square brackets, \texttt{[]}. For example if the MTConnect Element is:

\xml{<Linear name='X'>...</...>}

The OPC/UA Object with browse name \xml{Linear[X]} will be created with the HasTypeDefinition 
referencing the \mtmodel{Linear} OPC/UA type. 
...
More information on the MTConnect Components can be found in \cite{MTCPart2}.
...
The \mtuatype{MTCompositionType} is the abstract supertype of the dynamically generated
composition types based on the attribute \gls{type} of the \gls{Composition} element
of the MTConnect \gls{MTComponent}. The \gls{Composition} is then related to the \glspl{MTDataItem} that reference the \gls{Composition}'s mtmodel{id} in their \mtmodel{compositionId} 
attribute. 
...
The abstract type for all data items types that are specifically for \mtmodel{CONDITION} category.
...
All Data Items with Category \mtmodel{EVENT} having Controlled Vocabularies (Enumerations) 
...
Represents the \mtmodel{category} attribute of the MTConnect \gls{MTDataItem}.
...
\end{table} 
\FloatBarrier
\item \texttt{SourceData::String:} The text that is the \mtmodel{CDATA} of the \xmlelement{Source} element.
...
\begin{itemize}
  \item If the value of the data item is \mtmodel{UNAVAILABLE} a status code of \mtmodel{Uncertain_NoCommunicationLastUsable}
  \item When a reset trigger is specified, new \mtmodel{Good_} status codes will be created. See \mtmodel{ResetTrigger} enumeration.
\end{itemize}
...
\begin{itemize}
\item \texttt{SignificantDigits::UInt16:} The significant digits is converted to a \uamodel{ValuePrecision}.
...
These need to become \mtmodel{Good_} status code in OPC UA.
...
The base type class for all data items with a category of \mtmodel{EVENT}.
...
in the message. The type is currently sub-typed from the OPC UA \mtmodel{SystemEventType} and 
...
All data items with category \mtmodel{EVENT} and a numeric value. These are usually counters for 
parts and lines. Currently only builtin types that are known to be integers will be
sub-typed from this type. Extended types will be subtyped from the \mtmodel{MTStringEventType}.
...
The \uamodel{EURange} will becreated if the \glspl{Constraint} element exists and both \mtmodel{Maximum} 
...
All data items with category \mtmodel{EVENT} where the data is freeform text. The data type
...
event types inherited from \mtmodel{MTEnumeratedEventType} (see \ref{type:MTControlledVocabEventType}).
...
The condition type is a derived from the UA \gls{ConditionType}. The Normal state of 
the condition is indicated by the ActiveState if \uamodel{False}. The severity 
is used to represent the MTConnect condition states of Warning and Fault with the values of
500 and 1000 respectively. 

An MTConditionType instance will be created for event MTConnect \gls{MTDataItem} with a 
\mtmodel{category} of \mtmodel{CONDITION}. The \mtuatype{MTConditionType} instance will be instantiated 
...
\gls{MTDataItem} types with \mtmodel{MTCondition} appended as a suffix. For example the 
...
If a \mtmodel{Source} of the \gls{MTDataItem} is provided, then the \mtmodel{HasCondition} relationship 
will be from the source \gls{MTDataItem} to the \mtmodel{MTConditionInstance}. Otherwise it will be 
...
\begin{itemize}
\item \texttt{NativeCode::String:} When instantiated in the address space this will represent the \uamodel{NativeCode} of the last
\gls{Event} that was received. When the ActiveState becomes False and becomes 
inactive, then the \uamodel{NativeCode} will be cleared.

\item \texttt{NativeSeverity::String:} When instantiated in the address space this will represent the \uamodel{NativeSeverity} of the last
\gls{Event} that was received. When the ActiveState becomes False and becomes 
inactive, then the \uamodel{NativeSeverity} will be cleared.
...
the representation of the data item type will be derived from the \mtmodel{BaseConditionClassType}. 
...
For example: The \dataitem{CONTOLLER_MODE} will be given as the \mtmodel{ControllerModeType}. 
...
Section 8.1 of the MTConnect Standard Part 2 \cite{MTCPart2} and the definition of the values can be found in Section 5.3 of MTConnect Standard Part 3 \cite{MTCPart3}. 

The description of each type was copied from the MTConnect Standard,
but this not the definitive text for each type. For authoritative normative text, please refer to the sources \cite{MTCPart2} and \cite{MTCPart3}.
...
The base type class for all data items with a category of \mtmodel{SAMPLE}.
...
Section 8.2 of the MTConnect Standard Part 2 \cite{MTCPart2} and the definition of the values can be found in Section 5.5 of MTConnect Standard Part 3 \cite{MTCPart3}. 
...
type \uamodel{MTEnumeratedEventType} derived from the OPC UA type
...
The current mode of the \mtmodel{MTController} component.
...
An indication of the operating condition of a mechanism represented by a \gls{Composition} type element.
...
Typically, the \mtmodel{FUNCTIONAL_MODE} SHOULD be modeled as a data item for the Device element, but 
...
Describes the operational relationship between a \mtmodel{PATH} \textit{Structural Element} and another \mtmodel{PATH} \textit{Structural Element} for pieces of equipment comprised of multiple logical groupings of controlled axes or other logical operations. 
...
perform its functions.
	...
	DEPRECATION WARNING: \mtmodel{PowerState} may be deprecated in the future.
...
The root type for all of the event types that have numeric CDATA.
...
The line of code or command being executed by a \mtmodel{Controller} \textit{Structural Element}.
...
The sub-types are related to the various \glspl{Variable} using the \mtuatype{HasMTSubClassType}. 
These provide the the second level type classifiaction of the MTConnect \glspl{MTDataItem} and 
also the \uamodel{ConditionSubClassId} for the \glspl{Condition}.
...
\FloatBarrier
	...
	Example: When multiple locations on a piece of bar stock are referenced as the indication for the \mtmodel{END_OF_BAR}, the additional location(s) MUST be designated as \mtmodel{AUXILIARY} indication(s) for the \mtmodel{END_OF_BAR}. 
...
An indication of the position of a mechanism that may move in a lateral direction. The mechanism is represented by a \mtmodel{Composition} type component.
...
Specific applications MAY reference one or more locations on a piece of bar stock as the indication for the \mtmodel{END_OF_BAR}. The main or most important location MUST be designated as the \mtmodel{PRIMARY} indication for the \mtmodel{END_OF_BAR}. 
...
The value of a signal or calculation issued to adjust the feedrate of the axes associated with a \mtmodel{Path} component when the axes, or a single axis, are operating as specified by a logic or motion program or set by a switch.
...
The value of a signal or calculation issued to adjust the feedrate of the axes associated with a \mtmodel{Path} component when the axes, or a single axis, are being operated in a rapid positioning mode or method (rapid). 
...
A \gls{MTDataItem} is representated in OPC UA as a sub-type of the most appropriate \mtmodel{BaseDataVariableType}. 
...
can be associated based on the data item category and type, the \mtmodel{Type Factory} will 
...
that are related to the using the \gls{mixes-in} dependency. Mixins allow for
